...
# neuron.py : implementazione di un neurone in Python
#
...
class Neuron():
	...
	def __init__(self,number_of_inputs,		\
		...
		self.inputs = []		# gli inputs del neurone
		self.output = 0			# l'output del neurone
		
		self.input = []			# l'input dal dataset
	...
	def reset(self):
		...
		self.delta = 0.0
		self.random_weights_and_bias()
		
	def random_weights_and_bias(self):
		# generates n random weights (-1,1) and a random bias (0,1)
		for i in range(self.number_of_inputs):
			self.weights.append(random.uniform(-1,1))	# pesi delle n connessioni relative agli n inputs della rete
